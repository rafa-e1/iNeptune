...
class MainViewController: UIViewController {
    ...
    @objc private func scissorsButtonDidTapped() {
        ...
        let randomImageView = SRP(rawValue: Int.random(in: 0...2))!
        computerImageView.image = UIImage(
            named: srpImageData[randomImageView.rawValue]
        )
        updateStatusLabel(
            playerChoice: .scissors,
            computerChoice: randomImageView
        )
    }
    ...
    private func updateStatusLabel(playerChoice: SRP, computerChoice: SRP) {
        if playerChoice == computerChoice {
            statusLabel.text = "비겼습니다!"
        } else if playerChoice == .scissors && computerChoice == .paper ||
                    playerChoice == .rock && computerChoice == .scissors ||
                    playerChoice == .paper && computerChoice == .rock
        {
            statusLabel.text = "이겼습니다!"
        } else {
            statusLabel.text = "졌습니다!"
        }
    }
